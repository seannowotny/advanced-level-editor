...
{
    public enum PanelDirection { None = -1, Left = 0, Top = 1, Right = 2, Bottom = 3 };
    ...
    public class PanelManager : MonoBehaviour
    {
        ...
        [SerializeField]
        private RectTransform previewPrefab = null;

        private List<PanelCanvas> canvases = new List<PanelCanvas>(8);
        ...
        private PanelCanvas previewPanelCanvas;
        ...
        public void RegisterPanel(Panel panel)
        {
            ...
            {
                panels.Add(panel);
                panel.GetComponentInParent<PanelCanvas>().UnanchoredPanelGroup.AddElement(panel);
            }
        }
        ...
        public Panel DetachPanelTab(Panel panel, int tabIndex)
        {
            if (tabIndex >= 0 && tabIndex < panel.NumberOfTabs)
            {
                ...
                {
                    Panel newPanel = PanelUtils.Internal.CreatePanel(null, panel.Canvas, panel.Canvas.PanelPrefab);
        ...
        public void CancelDraggingPanel()
        {
            if (draggedPanel != null)
            {
                ...
                if (draggedPanel.Group is UnanchoredPanelGroup unanchoredGroup)
                {
        ...
        public void OnEndPanelTranslate(Panel panel)
        {
            if (draggedPanel == panel)
            {
                if (draggedPanel.RectTransform.parent != draggedPanel.Canvas.RectTransform)
                {
                    draggedPanel.RectTransform.parent.GetComponent<PanelCanvas>().UnanchoredPanelGroup.AddElement(draggedPanel);
                }
        ...
        {
            RectTransform previewPanel = Instantiate(previewPrefab);
#if UNITY_EDITOR
            previewPanel.gameObject.name = "Panel Drag Prefab";
#endif
